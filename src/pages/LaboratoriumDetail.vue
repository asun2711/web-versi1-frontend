        ...
        <div class="card-utama">
          <div class="sub-cards">
            ...
            <template v-for="item in latestIGD" :key="`igd-${item.id}`">
              <div class="sub-card" @click="goToDetail('rawatigd', item.id)">
...
import type { Laboratorium, RawatJalan, RawatInap, Igd, Radiologi, Mcu } from '@/services/api';

export default defineComponent({
  name: 'LaboratoriumDetail',
  ...
  setup() {
    ...
    const detail = ref<Laboratorium | null>(null);
    ...
    // Computed properties
    const titleText = computed(() => detail.value?.namalaboratorium || '');
    ...
    const fetchDetail = async () => {
      ...
      if (!id) return;
      try {
        const res = await laboratoriumApi.getById(id);
        detail.value = res;
        window.scrollTo(0, 0);
      } catch (err) {
        console.error('Gagal memuat detail laboratorium:', err);
      }
    };
